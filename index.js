...
if (!fs.existsSync(inFilePath)) {
  notify({
    m: "Error: In file does not exists...",
    v: inFilePath,
    d: ["red", "bold"],
  });
  process.exit(0);
...
const urls = [
  ...new Set(fs.readFileSync(inFilePath, "utf-8").split("\n").filter(Boolean)),
];

function download(i) {
  let timeout = 0;
  const outPath = path
    .join(outDir, urls[i].split("/").pop())
    .replace(/:/g, "")
    .replace(/'/g, "")
    .replace(/"/g, "");
  ...
  if (skipList.includes(urls[i])) {
    ...
    try {
      spawnSync(
        "wget",
        ["--content-disposition", `"${urls[i]}"`, "-P", outDir],
